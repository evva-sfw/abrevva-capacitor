...
describe('AbrevvaBLEClient', () => {
  ...
  it('should run stopEnabledNotifications', async () => {
    ...
    expect((AbrevvaBLEClient as unknown as AbrevvaBLEClientWithPrivate).eventListeners.get('onEnabledChanged')).toBeUndefined();
  });
  ...
  it('should run connect without disconnect callback', async () => {
    ...
    expect((AbrevvaBLEClient as unknown as AbrevvaBLEClientWithPrivate).eventListeners.get('disconnected|123')).toBeUndefined();
    expect(AbrevvaBLE.connect).toHaveBeenCalledWith({ deviceId: mockDevice.deviceId });
  });
  ...
  it('should run connect with timeout', async () => {
    await AbrevvaBLEClient.connect(mockDevice.deviceId, () => undefined, { timeout: 20000 });
    expect(AbrevvaBLE.connect).toHaveBeenCalledWith({ deviceId: mockDevice.deviceId, timeout: 20000 });
  });

  it('should run disconnect', async () => {
    await AbrevvaBLEClient.disconnect(mockDevice.deviceId);
    expect(AbrevvaBLE.disconnect).toHaveBeenCalledWith({ deviceId: mockDevice.deviceId });
  });
  ...
  it('should run read', async () => {
    ...
    expect(AbrevvaBLE.read).toHaveBeenCalledWith({ deviceId: mockDevice.deviceId, service, characteristic });
  });

  it('should run read with timeout', async () => {
    (AbrevvaBLE.read as jest.Mock).mockReturnValue({ value: '00 05 c8 ' });
    await AbrevvaBLEClient.read(mockDevice.deviceId, service, characteristic, { timeout: 6000 });
  ...
  it('should run write with timeout', async () => {
    (Capacitor.getPlatform as jest.Mock).mockReturnValue('android');
    await AbrevvaBLEClient.write(mockDevice.deviceId, service, characteristic, numbersToDataView([0, 1]), { timeout: 6000 });
    expect(AbrevvaBLE.write).toHaveBeenCalledWith({
      deviceId: mockDevice.deviceId,
      service,
      characteristic,
  ...
  it('should run disengage on native platform', async () => {
    ...
    (AbrevvaBLE.disengage as jest.Mock).mockReturnValue({ value: 'ACCESS_STATUS_AUTHORIZED' });
  ...
  it('should run startNotifications', async () => {
    ...
    expect((AbrevvaBLEClient as unknown as AbrevvaBLEClientWithPrivate).eventListeners.get(key)).toBe(mockPluginListenerHandle);
  });
